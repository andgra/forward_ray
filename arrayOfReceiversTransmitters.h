...
//                    if (IsPointOnEdge(curBeam.startPoint, l))
                    ...
                    if (x >= l.xStart && x <= l.xEnd && y >= l.yStart && y <= l.yEnd)//точка лежит на отрезке
                    {
                        ...
                        if ((curBeam.direction.X * (x - curBeam.startPoint.X) >= 0) &&
                            ...
                            if (fabs(currentDistance) < 1e-5) continue; //расстояние 0
    ...
    void SendBeam(beam curBeam, queue<beam> &calculationQueue)//луч - начальная точка, угол, амплитуда
    {
        ...
        if (closestIntersection.figureIndex == 0)//это не фигура, а один из концов пространства
        {
            ...
            abs = GetAbsorption(envAbs, distance);
...
//                else//для горизонтальной волны не должен меняться знак - там нет отражений
                    calculationQueue.push(beam(curBeam, closestIntersection.intersectionPoint,
                                               vec2f::reflect(curBeam.direction, vec2f(0, 1)), time, val, 0));
...
//                                           vec2f::reflect(curBeam.direction, vec2f(0, -1)), time, val, 0));
        ...
        { //проходящая волна скользит по поверхности вместо прохождения насквозь - мы не считаем эту волну
            ...
            float k = (sqrt((v2 * v2 - v1 * v1) / (temp * temp) + 1) - 1);
...
// List<DifEffectInstance> diffactions;
    ...
    vector<vector<float>> MakeConvolution() {
        ...
        int maxSize = 4096;
        comp *tempColumn;
        auto impulseSpec = new comp[maxSize];
        for (int i = 0; i < signal.size(); i++) {
            //if (i < impulse.Length)
            impulseSpec[i] = comp(signal[i]);
        }
        for (int i = signal.size(); i < maxSize; i++) {
            //if (i < impulse.Length)
            impulseSpec[i] = comp(0);
        }

        FFT(impulseSpec, maxSize);
        for (int x = 0; x < width / step; x++) {
            tempColumn = new comp[maxSize];
            for (int i = 0; i < min(maxSize, maxTime); i++) {
                tempColumn[i] = comp((double)recordedData[x][i]);
            }
            for (int i = min(maxSize, maxTime); i < maxSize; i++) {
                tempColumn[i] = comp(0);
            }
            Convolution(tempColumn, impulseSpec, maxSize);

            for (int i = 0; i < maxTime; i++)//обрезаем то, что вылезло за границы
            {
                if (i < maxSize) {
                    convolvedData[x][i] = (float) tempColumn[i].real() / maxSize;
//                    convolvedData[x][i] = (float) sqrt(tempColumn[i].real() * tempColumn[i].real() + tempColumn[i].imag() * tempColumn[i].imag()) / maxSize;//(4096 * 4096);
                } else {
                    convolvedData[x][i] = 0;
    ...
    void Convolution(comp *tempColumn, comp *impulseSpec, unsigned int size)//спектр импульса уже умноженный на себя
    {
        FFT(tempColumn, size);
...
//        string funcStr = serialize<complex>(tempColumn, size);
    ...
    void DrawDif(pointF p, float val, float prevTime) {
        if (p.Y >= figureCollection[0].up) return; // точки на верхней границе всей области не рассматриваем
        ...
        while (intersection.intersectionPoint.Y <= p.Y) {
            ...
            float curAbs = GetAbsorption(figureCollection[curBeam.figureIndex].absorption_c,
            ...
            curBeam = beam(curBeam, intersection.intersectionPoint, vUp, curBeam.time + curTime,
                           curBeam.value * curAbs,
                           curFigureIndex);
            intersection = FindClosestIntersection(curBeam, false);
        }

        curTime = curBeam.time;
        curVal = curBeam.value * val * valKoef;

        float delta = (float) (p.Y * dY / speed / dt - curTime);//разность между расчетным и реальным временем
...
//#pragma omp critical
        {
            recordedData[position][(int) round(time)] += curVal * 2;
        }
        ...
        for (int i = 1; stop != true; i++) {
            stop = true;
            newTime = (float) ((sqrt(pow(step * i * dX, 2) + pow(p.Y * dY, 2)) / speed) / dt) - delta + prevTime;
            if (newTime > maxTime)
                return;
            if (position + i < coordinates.size()) {
//#pragma omp critical
                {
                    recordedData[position + i][(int) round(newTime)] += curVal / i;
                }
                stop = false;
            }
            if (position - i >= 0) {
